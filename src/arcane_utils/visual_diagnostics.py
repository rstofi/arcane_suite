...
plt.rcParams['axes.autolimit_mode'] = 'round_numbers' #Set automatic axis limits nicely
...
def create_field_ID_RA_Dec_plot(mspath,
    ...
    logger.debug("Selected field ID's: {0:s}".format(misc.convert_list_to_string(
                                            list(phase_centre_ID_dict.keys()))))
    ...
    plt.savefig(otfig_path,bbox_inches='tight')
    plt.close()
